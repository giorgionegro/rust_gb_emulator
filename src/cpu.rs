...
impl Registers {
    ...
    pub fn write_8(&mut self, register: char, value: u8) {
        ...
        match register {
            ...
            'f' => self.af = (tmp.af & 0xFF00) | ((value & 0xF0) as u16), // Lower 4 bits of F are always 0
            'i' => self.ime = value,
            _ => (),
        }
    }

    pub fn write_16(&mut self, register: &str, value: u16) {
        match register {
            "af" => self.af = value & 0xFFF0, // Lower 4 bits of F are always 0
            "bc" => self.bc = value,
            "de" => self.de = value,
            "hl" => self.hl = value,
            "sp" => self.sp = value,
...
const OPCODE_LENGTHS: [u8; 256] = [
    ...
    // 0x10-0x1F (0x10 STOP is 2 bytes)
...
impl Cpu {
    ...
    pub fn step(&mut self, mem: &mut Memory) -> u32 {
        ...
        static mut PREV_PC: u16 = 0;
        static mut STUCK_COUNT: u32 = 0;
        static mut PC_LOG_COUNT: u32 = 0;

        unsafe {
            PC_LOG_COUNT += 1;

            // Log everything when PC < 0x10 or we're about to jump there
            if pc < 0x0010 {
                let sp = self.registers.read_16("sp");
                let a = self.registers.read_8('a');
                let f = self.registers.read_8('f');
                println!("PC=0x{:04X}, opcode=0x{:02X}, SP=0x{:04X}, A=0x{:02X}, F=0x{:02X}, PREV_PC=0x{:04X}",
                    pc, opcode, sp, a, f, PREV_PC);

                // If this is a RET, show what's on the stack
                if opcode == 0xC9 || opcode == 0xC0 || opcode == 0xC8 || opcode == 0xD0 || opcode == 0xD8 || opcode == 0xD9 {
                    let ret_addr = mem.read_16(sp);
                    println!("  RET instruction! Will return to: 0x{:04X}", ret_addr);
                }
            }

            // Log when we're about to execute an instruction that might jump to 0
            if PREV_PC >= 0x0700 && PREV_PC < 0x0800 {
                // Check for RET instructions
                if opcode == 0xC9 {
                    let sp = self.registers.read_16("sp");
                    let ret_addr = mem.read_16(sp);
                    if ret_addr < 0x0010 {
                        println!("WARNING: RET at PC=0x{:04X} will jump to 0x{:04X}! SP=0x{:04X}", pc, ret_addr, sp);
                    }
                }
            }
    ...
    fn ld_r1_r2(&mut self, mem: &mut Memory, dest: &str, source: &str) {
        ...
        if dest.contains("(") { //This is terrible for performance but oh well for now it will do
            dstmem = true;
    ...
    fn ld_sp_e(&mut self, mem: &mut Memory) {
        ...
        let offset = mem.read_8(self.registers.read_16("pc") + 1) as i8;
        let sp = self.registers.read_16("sp");
        let result = sp.wrapping_add(offset as i16 as u16);
        self.registers.write_16("hl", result);
    ...
    fn ldh_a_n(&mut self, mem: &mut Memory) {
        let offset = mem.read_8(self.registers.read_16("pc") + 1);
    ...
    fn inc_r(&mut self, _mem: &mut Memory, reg: &str) {
        if reg.len() == 1 {
            let value = self.registers.read_8(reg.chars().next().unwrap());
            let result = value.wrapping_add(1);
            self.registers
                .write_8(reg.chars().next().unwrap(), result);
    ...
    fn dec_r(&mut self, _mem: &mut Memory, reg: &str) {
    ...
    fn inc_m(&mut self, mem: &mut Memory, reg: &str) {
        let addr = self.registers.read_16(reg);
    ...
    fn add_hl(&mut self, _mem: &mut Memory, reg: &str) {
        let value = self.registers.read_16(reg);
    ...
    fn add_sp_e(&mut self, mem: &mut Memory) {
        ...
        let sp = self.registers.read_16("sp");
        let result = sp.wrapping_add(offset as i16 as u16);
        self.registers.write_16("sp", result);
    ...
    fn sbc_a_r(&mut self, mem: &mut Memory, reg: &str) {
        ...
        let a = self.registers.read_8('a');
        let carry_in = if (self.registers.read_8('f') & CARRY_FLAG) != 0 { 1u8 } else { 0u8 };

        let temp = a as u16;
        let temp_result = temp.wrapping_sub(value as u16).wrapping_sub(carry_in as u16);
    ...
    fn daa(&mut self, _mem: &mut Memory) {
        ...
        let flags = self.registers.read_8('f');
        ...
        self.registers.write_8('f', new_flags);
        self.registers.write_8('a', value);
    }
    ...
    fn halt(&mut self, mem: &Memory) {
        ...
        if self.registers.ime == 0 && interrupt_pending {
    ...
    fn call_cb(&mut self, mem: &mut Memory) {
        ...
        match cb_opcode {
            0x00..=0x07 => self.rlc_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x08..=0x0F => self.rrc_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x10..=0x17 => self.rl_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x18..=0x1F => self.rr_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x20..=0x27 => self.sla_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x28..=0x2F => self.sra_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x30..=0x37 => self.swap_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x38..=0x3F => self.srl_r(mem, REG_NAMES[(cb_opcode & 0x07) as usize]),
            0x40..=0x7F => {
                self.bit_n_r(
                    mem,
                    REG_NAMES[(cb_opcode & 0x07) as usize],
                    (cb_opcode >> 3) & 0x07,
                );
            }
    ...
    fn jr_e(&mut self, mem: &mut Memory) {
        ...
        self.registers.write_16("pc", target);
    }
    ...
    fn jr_f_e(&mut self, mem: &mut Memory, cflag: char, z: bool) {
        ...
        if (self.registers.read_8('f') & flag) >> shift == cond {
            ...
            let pc = self.registers.read_16("pc");
            self.registers.write_16("pc", pc + 2);
        }
    }

    fn jp_nn(&mut self, mem: &mut Memory) {
        let target_address = mem.read_16(self.registers.read_16("pc") + 1);
        self.registers.write_16("pc", target_address);
    }
    ...
    fn jp_f_nn(&mut self, mem: &mut Memory, cflag: char, condition: bool) {
        ...
        if (self.registers.read_8('f') & flag) >> shift == cond {
            ...
            let pc = self.registers.read_16("pc");
            self.registers.write_16("pc", pc + 3);
        }
    }
    ...
    fn call_nn(&mut self, mem: &mut Memory) {
        let target_address = mem.read_16(self.registers.read_16("pc") + 1);
        let return_address = self.registers.read_16("pc") + 3; // Return to instruction after CALL

        // Push return address onto stack
        self.registers.write_16("sp", self.registers.read_16("sp") - 2);
        mem.write_16(self.registers.read_16("sp"), return_address);
    ...
    fn rst(&mut self, mem: &mut Memory, value: u16) {
        let return_address = self.registers.read_16("pc") + 1; // RST is 1 byte

        // Push return address onto stack
        let sp = self.registers.read_16("sp");
        self.registers.write_16("sp", sp.wrapping_sub(2));
        mem.write_16(self.registers.read_16("sp"), return_address);
    ...
    fn ret_f(&mut self, mem: &mut Memory, cflag: char, z: bool) {
        ...
        if (self.registers.read_8('f') & flag) >> shift == cond {
            ...
            let pc = self.registers.read_16("pc");
            self.registers.write_16("pc", pc + 1);
        }
    }
    ...
    fn reti(&mut self, mem: &mut Memory) {
        ...
        self.registers.write_16("sp", self.registers.read_16("sp") + 2);
        self.registers.write_16("pc", value);
        self.registers.ime = 1;  // Re-enable interrupts
    }
    ...
    pub fn execute(&mut self, opcode: u8, mem: &mut Memory) {
        ...
        match opcode {
            ...
            0x40..=0x75 | 0x77..=0x7F => {
                ...
                let dest_idx = ((opcode >> 3) & 0x07) as usize;
                let src_idx = (opcode &  0x07) as usize;
                self.ld_r1_r2(mem, REG_NAMES[dest_idx], REG_NAMES[src_idx]);
            }
            0x80..=0x87 => self.add_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0x88..=0x8F => self.adc_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0x90..=0x97 => self.sub_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0x98..=0x9F => self.sbc_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0xA0..=0xA7 => self.and_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0xA8..=0xAF => self.xor_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0xB0..=0xB7 => self.or_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            0xB8..=0xBF => self.cp_a_r(mem, REG_NAMES[(opcode & 0x07) as usize]),
            ...
            _ => {
                println!("CPU: Unknown/unimplemented opcode 0x{:02X} at PC 0x{:04X}!", opcode, self.registers.read_16("pc"));
    ...
    pub fn handle_post_instruction(&mut self, mem: &mut Memory, opcode: u8, _length: u64) -> u32 {
        ...
        if !pc_modifying_opcodes.contains(&opcode) {
            ...
            if self.halt_bug {
                ...
                self.registers.write_16("pc", pc.wrapping_add(length));
            }
        }
    ...
    fn service_interrupt(&mut self, mem: &mut Memory, interrupt: u8) {
        ...
        let sp = self.registers.read_16("sp");
        self.registers.write_16("sp", sp.wrapping_sub(2));
        mem.write_16(self.registers.read_16("sp"), pc);

        // Jump to interrupt vector
        let vector = 0x0040 + (interrupt as u16 * 0x08);
        self.registers.write_16("pc", vector);
